...
function addLatLng(event) {
						...
						if ((status == google.maps.GeocoderStatus.OK)
							...
							if (results[0]) {
								...
								//alert( results[0].formatted_address );
								 markerStart = createMarker(event.latLng, "Posisi Awal", stringForm(contentString,"noprice",1), "green","out","window",1);
								  origin = event.latLng;
							}
						} else
  ...
  directionsService.route(request, function(response, status) {
		if (status == google.maps.DirectionsStatus.OK) {
			...
			RenderCustomDirections(response,"window");
		}
  });
...
function stringForm(alamat,action,id){
	var addition = "";
	var keterangan="";
	var price="";
	if(ket[id]!=null) {
		keterangan = ket[id].keterangan;	
	/*if(ket[id].price!=null){
		price = ket[id].price;	
	}
	if(ket[id].address!=null){
		alamat = ket[id].address;
	}*/
	}
	if(action=="price") addition="<br><label>Harga</label><br><input type='text' name='price' value='"+price+"'>";
	var form = "<br><form action='#' method='post'>"+
			//"<input type='hidden' id='" + id + "'>" +			
			"<label>Keterangan</label><br><textarea id='keterangan' name='keterangan' style='width:200px; height:40px'>"+keterangan+"</textarea>" +
			addition+"<br>" +
			"<label>Alamat</label><br><textarea name='alamat' style='width:200px; height:70px'>"+alamat+"</textarea><br> " +
			"<input type='button' value='Hapus'>" +
			"&nbsp;&nbsp;<input type='button' id='tutup' value='Tutup' onclick='tutupInfWindow("+alamat+","+action+","+id+")'>" +
			"</form>";
...
function tutupInfWindow(alamat,action,id){
	alert('tes');
}
...
function RenderCustomDirections(response,windowmode) {
	...
	for (var i = 0; i < legs.length; i++) {
		if (i == 0) {
			...
			startLocation.marker = createMarker(legs[i].start_location,"Start", stringForm(legs[i].start_address,"noprice",1), "green","in",windowmode,1);
			
		}
		else if(i!=0&&i<legs.length)
			{
			if (typeof waypts[i] != 'undefined') waypts[i].marker.setMap(null);
			waypts[i]=new Object();
			waypts[i].latlng=legs[i].start_location;
			waypts[i].marker=createMarker(legs[i].start_location,"Cekpoin "+i, stringForm(legs[i].start_address,"price",i+1), "green","out",windowmode,i+1);
	...
	endLocation.marker = createMarker(endLocation.latlng, "Posisi Akhir",stringForm(endLocation.address,"price",99), "red",echo,windowmode);
...
$('#simpan').click(function(){
	$.ajax({
		  type: "POST",
		  url: "./../assets/ajax/saveRoute.php",
		  data: { rleg: JSON.stringify(directionsDisplay.directions.routes[0].legs)}
		})
		  .done(function( msg ) {
...
function changeContent(id,content){
	var markerTemp;
	if(id==1) markerTemp=startLocation.marker;
	else if(id==99) markerTemp=endLocation.marker;
	else markerTemp = waypts[id].marker;
...
var label = [];
function createMarker(latlng, label, html, color, action, windowmode, id) {
	var contentString = '<b id="label">' + label + '</b><br>' + html;
	...
	//changeContent(id,contentString);
	...
	google.maps.event
			.addListener(
					...
					function() {
													...
													if (results[0]) {
														contentString = '<b>'
																+ label+
																 '</b><br />'+/*
																+ results[0].formatted_address;*/
														stringForm(results[0].formatted_address,action);
													}
												} else
													alert("Geocoder failed due to: "
															+ status);
						...
						if(destination!=null) destination=endLocation.marker.getPosition();
						var request = {
							origin : startLocation.marker.getPosition(),
