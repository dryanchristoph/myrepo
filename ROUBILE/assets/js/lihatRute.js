...
function stringForm(label2,alamat,action){
	...
	if(action!="noprice") addition="<br><label>Harga</label><br><a>"+harga2+"</a>";
	var form = "<br><form action='#' method='post'>";
	if(label2=='Posisi Awal'||label2=='Posisi Akhir'){
		form+= "<label>Nama Lokasi</label><br><a>"+lokasi2+"</a><br>";
	}
	form+="<label>Keterangan Angkot</label><br><a>"+ket2+"</a>" +
			addition+"<br>" +
			"<label>Alamat</label><br><a>"+alamat+"</a><br> " +
...
function createMarker(latlng, label, html, color, action, windowmode) {
	...
	var contentString = '<div style="font-size:85%"><b id="label" style="font-size:x-large">' + label + '</b><br>' + html;
