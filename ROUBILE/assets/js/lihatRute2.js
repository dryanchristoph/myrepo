...
function initialize() {
	...
	google.maps.event.addListener(directionsDisplay, 'directions_changed', function(event) {
		...
		/*for(var i=0;i<rleg.length;i++){
			alert('Stopover = '+waypoints.length+'<br>Waypoints = '+waypts.length+'<br>'+steps.length);			
		}*/
		//alert(JSON.stringify(waypts[1], null, 4));
		//alert(JSON.stringify(waypts.join("\n"), null, 4));
		/*for (i=0;i<gfd.length;i++){
			alert(JSON.stringify(event, null, 4));
		}*/
	});
	//$( '#toolbar' ).css( "margin", "340px 10%" );
		//$('[id="radio-del"]').button('disable');
	$("#keteranganTool").html('Klik map untuk tambah rute');
		//$("#divKeterangan").hide();
			//$("#divKeterangan").show();
			//$( '#toolbar' ).css( "margin", "280px 10%" );
			$("#keteranganTool").html('Klik map untuk tambah rute');
...
function addLatLng2(){
	var address1 = "101-119 Massachusetts Avenue, Institut Teknologi Massachusetts, Cambridge, Massachusetts 02139, Amerika Serikat";
	var address2 = "120 Sidney Street, Institut Teknologi Massachusetts, Cambridge, Massachusetts 02139, Amerika Serikat";
	waypoints.push({
		location : address1,
		stopover : true
	},{
	...
	var request = {
		...
		destination : "186 Brookline Street, Cambridge, Massachusetts 02139, Amerika Serikat",
		waypoints : waypoints,
		travelMode : google.maps.DirectionsTravelMode.WALKING
	};
	//directionsService.route(request, directionsDisplay);
						...
						if ((status == google.maps.GeocoderStatus.OK)
							...
							if (results[0]) {
								...
								//alert( results[0].formatted_address );
								 markerStart = createMarker(event.latLng, "Posisi Awal", stringForm(contentString), "green","out");
...
function stringForm(alamat,action){
	var addition = "";
	if(action=="price") addition="<br><label>Harga</label><br><input type='text' name='price'>";
	var form = "<br><form action='#' method='post'>"+
			"<label>Keterangan</label><br><textarea name='keterangan' style='width:200px; height:70px'>&nbsp;</textarea>" +
			addition+"<br>" +
...
function RenderCustomDirections(response) {	
	...
	for (var i = 0; i < legs.length; i++) {
		if (i == 0) {
			...
			startLocation.marker = createMarker(legs[i].start_location,"Start", stringForm(legs[i].start_address,"noprice"), "green","in");
		}
			...
			waypts[i].marker=createMarker(legs[i].start_location,"Cekpoin "+i, stringForm(legs[i].start_address,"price"), "green","out");
	...
	endLocation.marker = createMarker(endLocation.latlng, "Posisi Akhir",stringForm(endLocation.address,"noprice"), "red",echo);
...
function addMarker(marker,position,icon,contentString) {
	...
	icon.anchor = new google.maps.Point(5, 45);
	marker.setIcon(icon);
...
function createMarker(latlng, label, html, color, action) {
